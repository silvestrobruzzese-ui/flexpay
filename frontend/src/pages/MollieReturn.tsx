...
import { mollieApi } from '../services/api';

export default function MollieReturn() {
  ...
  const checkPayment = async () => {
    try {
      const data = await mollieApi.handleReturn(sessionId!);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-4 bg-gray-100">
        <div className="bg-white rounded-2xl shadow-2xl p-8 text-center max-w-md">
          ...
          <h1 className="text-xl font-bold text-gray-800 mb-2">Verifica pagamento...</h1>
          <p className="text-gray-500">Stiamo verificando lo stato del tuo pagamento</p>
        </div>
      </div>
    );
  }
